...
export interface TableProps<T> {
  ...
  /** Makes each row clickable (e.g. navigate to a detail page). Cells that need
   * their own click target (a link, a button) should call `stopPropagation`. */
  onRowClick?: (row: T, id: string) => void;
  /** Extra class names for a row's `<tr>` (e.g. dimming an archived record). */
