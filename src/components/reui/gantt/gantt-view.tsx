...
import { PlusSignIcon, MinusSignIcon, DragDropVerticalIcon, ArrowRight01Icon, ArrowLeft01Icon } from "@hugeicons/core-free-icons"
          ...
          <div className="flex h-full">
            {groups.map((group) => (
              <div
                ...
                // Deliberately quieter than the unit row below it: the units
                // are what bars are read against, the group only says which
                // quarter/month you are in.
                className="text-muted-foreground/80 flex min-w-0 items-center ps-3 pe-2 text-[10px] font-medium tracking-wide uppercase"
            ...
            {units.map((unit) => (
              <div
                ...
                className={cn(
                  ...
                  // the scanning row: a step up from the group label above it
                  "text-foreground/75",
            ...
            unitFractions
              ...
              .map(({ unit, start }) => (
                <span
                  ...
                  data-axis="vertical"
                  className={cn(
                    // softer than the header's own rules: in the body these
                    // run behind every row, and at full border weight a
                    // day-unit scale reads as a spreadsheet
                    "absolute inset-y-0 w-px",
                    gridLines.vertical === "dashed"
                      ? "bg-[repeating-linear-gradient(to_bottom,var(--color-border)_0,var(--color-border)_3px,transparent_3px,transparent_6px)]"
                      : "bg-border/60"
            ...
            >
              ...
              <TooltipProvider delay={600} closeDelay={0} timeout={300}>
                <Tooltip>
                  ...
                  >
                    <HugeiconsIcon icon={PlusSignIcon} strokeWidth={2} className="size-3" aria-hidden="true" />
                  </TooltipTrigger>
                ...
                <Tooltip>
                  ...
                  >
                    <HugeiconsIcon icon={MinusSignIcon} strokeWidth={2} className="size-3" aria-hidden="true" />
                  </TooltipTrigger>
                  <TooltipContent side="left">
 ...
 *
 * Primary, not destructive: it marks the same instant as the today column
 * wash, and a red rule would read as an alert state next to bars whose own
 * palette already uses red for trouble.
 */
...
/**
 * The now-line's cap, pinned INSIDE the sticky header at the header/body
 * boundary: it stays put while the line scrolls beneath the header. The
 * "Today" chip rides above it in the group row, so the line names itself
 * instead of leaving the reader to infer what the coloured column means.
 */
function GanttNowDot({
  ...
  return (
    ...
    >
      <span
        title={format(
          toZoned(now, settings.timeZone),
          settings.i18n.formats.dayTitle,
          { locale: settings.locale }
        )}
        className="bg-primary text-primary-foreground absolute -top-6 left-1/2 -translate-x-1/2 rounded-full px-1.5 py-px text-[10px] leading-4 font-medium whitespace-nowrap"
      >
        {settings.i18n.labels.today}
      </span>
      <span className="bg-primary absolute -bottom-0.75 left-1/2 size-1.5 -translate-x-1/2 rounded-full" />
    </span>
  )
}
        ...
        >
          ...
          >
            {onGripPointerDown && (
              ...
              >
                <HugeiconsIcon icon={DragDropVerticalIcon} strokeWidth={2} className="size-3" aria-hidden="true" />
              </button>
            )}
      ...
      <TooltipProvider delay={600} closeDelay={0} timeout={300}>
        {chips.map((chip) => (
          <Tooltip key={`${chip.id}-${chip.side}`}>
            ...
            >
              {chip.side === "start" ? (
                <HugeiconsIcon icon={ArrowLeft01Icon} strokeWidth={2} className="size-3" aria-hidden="true" />
              ) : (
                <HugeiconsIcon icon={ArrowRight01Icon} strokeWidth={2} className="size-3" aria-hidden="true" />
              )}
              <span
                aria-hidden
