...
function GanttBar<TData = unknown>({
    ...
    children: (
      <>
        ...
        {progress === 100 && !consumerOwnsContent && !milestone && (
          ...
          <HugeiconsIcon icon={Tick02Icon} strokeWidth={2} className="relative size-2.5 shrink-0 opacity-80" aria-hidden="true" />
        )}
        {content}
        {resizeHandles}
      </>
  ...
  // Same contract for the hover tooltip: the shell, the hover-only open
  // policy and the mid-gesture suppression stay here; only the content is
  // the consumer's. Returning null from the override means "no tooltip".
  const tooltipContent = viewConfig.renderEventTooltip ? (
    viewConfig.renderEventTooltip(renderProps)
  ) : (
    <>
  ...
  const barTree = (
    <TooltipProvider delay={500} closeDelay={0} timeout={300}>
      ...
      >
        {trigger}
        {tipOpen && !anyInteracting && tooltipContent !== null && (
