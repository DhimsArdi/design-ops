...
- **Transitions.dev** — `transitions-dev` + `transitions-polish` (installed from `github.com/Jakubantalik/transitions.dev` into `.claude/skills/`). Motion skills: 32 portable CSS transition recipes, and a five-dimension motion-token scale (duration / easing / distance / scale / blur) with rules for open/close asymmetry, hover in/out, stagger and intent delay. The token scale is installed in `src/app/globals.css` and is the source of truth for motion values — see `docs/DECISIONS.md` for which tokens were deliberately skipped to avoid colliding with Tailwind v4's own theme names. Commands: `transitions reveal` / `review` / `apply` / `refine` / `polish`. Like Anti-slop and Impeccable, it does **not** set visual direction: this is an enterprise planning app, so use it to tune motion that already exists, not to add decorative transitions the PRD doesn't ask for.
