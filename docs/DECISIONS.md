...
## `Designer.home_squad_id` became nullable, and Squad View's drag-move got a real bug fix (2026-09-14)

**Bug found:** dragging a Shared member out of a squad card in Teams → Squad View and choosing **Move** only ever wrote two things — the designer's new `home_squad_id`, and a cleanup of the *target* squad's `squad_designer_memberships` row (in case it made the new Primary Squad redundantly "Shared"). It never cleaned up the row for the card the designer was actually dragged out of. That's invisible when the card you drag from happens to be the designer's Home Squad (there's no membership row there to begin with), but wrong whenever it's a Shared card: the stale `squad_designer_memberships` row keeps the designer showing up on that card's board tile forever, while Squad Detail — which only ever reads `home_squad_id`, never this table — correctly stops listing them. Two screens, two different "who's in this squad" models, and the move only ever maintained one of them.

Root cause was one line: `handleDragEnd` (`squad-board.tsx`) read `sourceSquadId` off the drag payload only to check it against the drop target, then never passed it anywhere — `AllocationRequest` had no field for it at all. Fix: `AllocationRequest` gained `sourceSquadId`, and `designer-allocation-dialog.tsx`'s `handleMove` now calls `squadDesignerMembershipRepository.removeMembership(designer.id, sourceSquadId)` whenever the source isn't the target (in addition to its existing target-side cleanup). `removeMembership` was already a safe no-op when no row exists, so this needed no new repository code.

**Separately, the user asked for an explicit "remove from squad" action** — not just reassignment via drag or the Move dialog. For a Shared membership that's a pure subtraction (delete the `squad_designer_memberships` row), already well-supported by the existing membership model. For a *Primary* membership it isn't: `Designer.home_squad_id` was `string`, not nullable, and the PRD explicitly described it as "a single required field" in several places (Master Data → Squads' Add/Manage-members copy, the old domain type comment) — a designer could never be without a squad, only moved to another one.

Making a designer's *primary* removal genuinely optional — rather than just hiding the option for Primary members and reusing the existing Move flow — was a deliberate scope call, confirmed with the user rather than assumed, because it changes a stated domain invariant (`docs/PRD.MD` §4.3/§8.1) and its Postgres backing (`designers.home_squad_id uuid not null`), not just a UI affordance. Chosen over the narrower alternative (Remove only for Shared members, Primary members always require picking a destination) because the user explicitly asked for parity between the two removal cases.

What changed: `Designer.home_squad_id: string | null`; `supabase/schema.sql` and a new `supabase/migrations/007_designer_home_squad_nullable.sql` drop the `not null` constraint (the `references public.squads(id)` FK and its implicit RESTRICT-on-delete stay — an Unassigned designer still can't be created pointing at a squad that doesn't exist, and a squad still can't be deleted out from under a designer who has it as Home Squad). Master Data → Designers' Home Squad field switched from a bare "chosen or blocked" required select to the same `"__none__"` sentinel + explicit "Unassigned" `SelectItem` pattern already used by Department/Stakeholder pickers (`profile-identity-fields.tsx`), rather than inventing a second convention for the same problem.

Every existing `squadsById.get(designer.home_squad_id)` / `squadRepository.getById(designer.home_squad_id)` call site across People, Overview, Teams, Projects' assignment pickers, and Squad Detail needed a null-guard (`designer.home_squad_id ?? ""` for `Map.get`, a ternary for the strictly-typed repository `getById`) — none of them needed new fallback UI, since they already rendered `?? "–"`/`?? "Unassigned"` defensively (a designer's home squad could already fail to resolve if `squadsById` was momentarily stale); those fallbacks just became reachable through a legitimate state instead of only a hypothetical one. Standardized the copy on "Unassigned" specifically (not "–", "Not set", or "no squad") wherever a Home Squad is genuinely absent, matching what Add Squad Members already showed for exactly this state.

**Remove from squad** itself was kept as two separate, unshared implementations — Teams → Squad View's board (a hover-revealed `X` per `DraggableDesignerRow`, confirmed via the existing shared `ConfirmDialog`) and Master Data → Squads' `ManageSquadMembersDialog` (a `DropdownMenuItem` next to Move, confirmed via an inline step in the same Dialog, matching its own "moving/removing/adding are steps, not second modals" rule rather than stacking `ConfirmDialog` on top of it) — rather than one shared mutation helper, because the two surfaces already duplicate their own Move logic (clear the Squad Lead if applicable, then write `home_squad_id`) independently and adding a third cross-cutting abstraction for one more near-identical mutation wasn't worth it yet.

## A squad's Designers/Members count now excludes its own Squad Lead — but only where the lead is already shown as its own field (2026-09-14)

**Reported bug (with screenshot):** Teams → Squad View's card showed a squad's `Squad Lead` in its own section, then listed that same person again inside `Designers (N)` below — "Squad Dimas" showed `Designers (2): Aci, Dimas Ardirosatama` even though Dimas was already named as the lead above. Root cause: `Designer.home_squad_id` (what makes someone a "member") and `Squad.lead_designer_id` (who's the lead) are independent fields, and a lead's `home_squad_id` normally *does* point at their own squad, so they satisfied both. Nothing in the domain model or `squadSelectors.ts` excluded one from the other — `getSquadMembers` was always "everyone whose `home_squad_id` is this squad," full stop, and every screen that also shows a separate Squad Lead field inherited the same double count: Squad View's card, Squad Detail's Members list, Master Data → Squads' `N designers` cell, and Overview's Team Snapshot (`Lead: X · N Designers`).

**Decision:** treat "is the lead" as automatically implying "is a member," and stop re-listing/re-counting that person a second time specifically in the four places above, which all pair a `Squad Lead` field with an adjacent Designers/Members list or count. Added `getSquadDesignerRoster(squadId)` to `squadSelectors.ts` — `getSquadMembers` filtered to drop `squad.lead_designer_id` — rather than changing `getSquadMembers` itself, which stays the full-headcount source of truth for everything that isn't about avoiding this specific visual redundancy:

- **Teams' staffing filter** (`Staffing: staffed/unstaffed`) and **Squad View's search-by-designer-name matching** keep using `getSquadMembers` unfiltered — a squad with only a lead and no other designers is still staffed, and searching a lead's name should still surface their squad.
- **Cross-squad "Outgoing support"** on Squad Detail keeps computing from the full member list — a lead who's supporting another squad's project is real information, unrelated to how they're listed on their own squad's page.
- **Master Data's "Manage members" dialog** still lists the lead alongside everyone else (with a `Squad Lead` badge) — that's where `Make/Remove Squad Lead` and `Move to another squad` actions live, so hiding the lead there would remove the only place those actions are reachable per-row. Only the row's summary `N designers` cell that opens that dialog was changed to the excluding-the-lead count; §16 now calls out that the cell's `N` and the dialog's actual row count can differ by one when a squad has a lead, and why.

`docs/PRD.MD` §14.7 (Squad Overview example, Squad View), §14.8 (Squad Detail's Members example — no longer repeats the lead), and §16 (Master Data member count) were updated to match. See `docs/CHANGELOG.md`.

## `Designer.job_title` became a closed select ("Design Role"), reusing Profile's `design_role` list (2026-09-14)

**Ask:** on Master Data → Designers, replace the free-text `Job Title` field with a select so users pick from existing options instead of typing anything.

**What the options should be, and why this isn't a new axis:** `job_title` had no documented valid-value list anywhere in `docs/PRD.MD` — it was always just a free-text example ("Product Designer"). But `use-profile-identity-form.ts` (Settings/Onboarding) already writes a Profile's chosen `design_role` (`DESIGN_ROLES` in `src/lib/domain/enums.ts`: `Product Designer, UX Designer, UI Designer, UX Researcher, Design Lead, Design Manager, Design Ops, Department Head, Other`) straight into a newly-created Designer's `job_title` whenever `isDesignerRole` is true — i.e. whenever the chosen role isn't `Department Head` (which creates a Stakeholder instead, per the existing Design Role vs System Role split). So `DESIGN_ROLES` minus `Department Head` was already the de facto set of values `job_title` could take on via onboarding; this change makes that the actual closed list rather than a coincidence enforced nowhere.

**Decision:** added `DESIGNER_JOB_TITLES` / `DesignerJobTitle` to `src/lib/domain/enums.ts` — a distinct type from `DesignRole`, not a reuse of it, because the two lists differ by more than `Department Head`: the roster's live data (Aci/Galuh NAP → `"Designer"`, Siska → `"UX Writer"`) already used two titles outside `DESIGN_ROLES`, and restricting the select to `DESIGN_ROLES` minus `Department Head` would have made those rows unselectable in their own edit dialog until someone picked a new value. Final list: `Product Designer, Designer, UX Designer, UI Designer, UX Researcher, UX Writer, Design Lead, Design Manager, Design Ops, Other` — a superset of `DESIGN_ROLES` minus `Department Head`, so `use-profile-identity-form.ts`'s existing write (`job_title: designRole as DesignerJobTitle`) stays valid for every value that select can actually produce.

**Why not literally reuse `DesignRole`:** the two fields answer different questions on different entities — `design_role` is "what discipline does this *account* do" (can be `Department Head`, which is nonsensical for a Designer row), `job_title` is "what title does this *roster entry* show." Naming them the same type would make a future reader assume the lists must always move together; keeping them separate types (that today happen to overlap) means a value can be added to one without silently changing the other, e.g. if `job_title` later needs a title `design_role` shouldn't offer to a new account, or vice versa.

**Scope of the change:** `Designer.job_title` is typed `DesignerJobTitle`, not `string` (`src/lib/domain/types.ts`). Master Data → Designers' Add/Edit dialog swapped the free-text `Input` for a `Select` (same structure as the adjacent `Seniority` select), which also means the old "Job title is required" client-side validation was removed — a select always holds a valid value, same as `Seniority` needs none. The table column and every other label reading this field ("Job Title" in Teams' designer details sheet) were relabeled "Design Role" for consistency; screens that only ever showed the raw value with a different existing label (People's "Role" column, project team-picker secondary lines) needed no change. No `supabase/schema.sql` CHECK constraint was added — `job_title` already had none (unlike `seniority`, which does), adding a DB-level constraint to match is a separate call and out of scope here, and no migration is needed since every current value survives unchanged. `docs/PRD.MD` §8.1 and §15 updated to match. See `docs/CHANGELOG.md`.
